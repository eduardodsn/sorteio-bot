...
class Dashboard:
    ...
    def create_entries(self):
        ...
        self.create_button(login_field, pass_field, link_field, qnt_field)
        


    def create_button(self, login_field, pass_field, link_field, qnt_field):
        button = Button(self.screen, text='Iniciar bot', bg='white', fg='black', command=lambda: run_bot(login_field.get(), pass_field.get(), link_field.get(), qnt_field.get()))
    ...
    def generate_label(self, text, font_size, x, y):
        new_label = Label(self.screen, text=text, font=('Montserrat', font_size), bg=self.bg_color, fg=self.fg_color)
